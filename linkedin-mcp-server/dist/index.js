...
async function main() {
    ...
    const server = new Server({ name: "linkedin-mcp-server", version: "1.0.0" }, { capabilities: { tools: {} } });
        ...
        try {
            ...
            switch (name) {
                ...
                case "linkedin_get_profile": {
                    const { profile_url } = args;
                    result = await client.getProfile(profile_url);
                    break;
                }
                case "linkedin_search_people": {
                    const { query, company, title, location, count = 10 } = args;
                ...
                case "linkedin_get_conversation": {
                    const { conversation_id, count = 20 } = args;
                ...
                case "linkedin_get_posts": {
                    const { profile_url, count = 5 } = args;
